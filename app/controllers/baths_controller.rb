    ...
    respond_to do |format|
      if @bath.save
        ...
        format.turbo_stream do
          render turbo_stream: [
            # 作成したレコードをリストに追加
            turbo_stream.replace("baths-container", partial: "baths/index", locals: { baths: current_child.baths.order(bathed_at: :desc), grouped_baths: current_child.baths.group_by { |f| f.bathed_at.to_date }, bath_all_dates: (current_child.baths.any? ? (current_child.baths.minimum(:bathed_at).to_date..[ current_child.baths.maximum(:bathed_at).to_date, Date.current ].max).to_a.reverse : [ Date.current ]), current_child: current_child }),
